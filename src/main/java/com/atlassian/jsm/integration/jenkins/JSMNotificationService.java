...
public class JSMNotificationService {
    private final static String INTEGRATION_PATH = "/jsm/ops/integration/v1/json/jenkins";

    private final org.slf4j.Logger logger = LoggerFactory.getLogger(JSMNotificationService.class);
    ...
    private JSMNotificationRequest request;
    ...
    private boolean checkResponse(String res) {
        try {
            ResponseFromJSM response = mapper.readValue(res, ResponseFromJSM.class);
            if (StringUtils.isEmpty(response.error)) {
                consoleOutputLogger.println("Sending job data to Jira Service Management is done");
                return true;
            } else {
    ...
    private String sendWebhookToJSM(String data) {
        try {
            ...
            consoleOutputLogger.println("Sending job data to Jira Service Management...");
    ...
    protected boolean sendPreBuildPayload() {
        ...
        String response = sendWebhookToJSM(payload);
    ...
    public static class ResponseFromJSM {
